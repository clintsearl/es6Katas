...
describe('initialize a `Map`', function(){
    it('a `new Map()` is empty, has size=0', function() {
      //added .size to get the size of the map
    ...
    it('init a Map with `[[1]]` is the same as `map.set(1, void 0)`', function() {
      //just what it says above, they are the same size.
      let map1 = new Map([[1]]);
    ...
    it('init Map with multiple key+value pairs', function() {
      const pair1 = [1, 'one'];
      const pair2 = [2, 'two'];
      //nested the two variables in the map another []
    ...
    it('keys are unique, the last one is used', function() {
      ...
      const pair3 = [1, 'eins'];
      const pair4 = [2, 'two'];
      //removed the pairs1 and 2 because the assert wants to see just the two
    ...
    it('init Map from an Object, is a bit of work', function() {
      ...
      // passed all the things into .set 
      //key puts the 'x' in there and the obj[key]
